...
const PRODUCT_MAP: Record<string, { blobUrl: string; title: string; lang: 'en' | 'es' }> = {
  // Silent Success (EN)
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const firstName = customerName.split(' ')[0] || (lang === 'en' ? 'friend' : 'amigo/a');

    // URL temporal firmada (1 hora)
    const downloadUrl = `${blobUrl}?token=${import.meta.env.BLOB_READ_WRITE_TOKEN}`;
    ...
    const btnText   = lang === 'en' ? '📥 Download my E-Book' : '📥 Descargar mi E-Book';
    ...
    console.log(`✅ Email sent to ${customerEmail} for: ${title}`);
